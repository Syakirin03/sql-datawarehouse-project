...
1. **Source → Bronze:** `bronze.load_bronze` truncates each Bronze table and bulk-inserts the matching CSV, with no transformation.
2. **Bronze → Silver:** `silver.load_silver` truncates each Silver table, then re-loads it from Bronze while applying cleansing rules (see below).
3. **Silver → Gold:** `ddl_gold.sql` creates three views directly over Silver — no physical tables — so Gold always reflects the latest Silver load.
...
Full field-level definitions (data types, descriptions, examples) are documented in [`docs/data_catalog.md`](docs/data_catalog.md).

## Data Quality

[`tests/quality_check_silver.sql`](tests/quality_check_silver.sql) validates the Silver layer after each load:
...
4. **Build Gold views:**
   ...
   ```
5. **Validate:** run `tests/quality_check_silver.sql` and review any returned rows.
6. **Consume:** query `gold.dim_customers`, `gold.dim_products`, and `gold.fact_sales` directly, or connect a BI tool.
